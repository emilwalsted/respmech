...
def _emg_di(x0, x1, y0, amp, cyc, ph, carrier, n=640):
    """A clean diaphragm-EMG trace over the full width, amplitude-gated to the
    inspiratory half of a flow wave with the SAME (cyc, ph): flat during
    expiration, a smooth crescendo-decrescendo burst during each inspiration
    (the positive half of ``sin(2π·cyc·x + ph)``, i.e. under the flow crests).
    Bursts go upward (−y)."""
    pts = []
    for i in range(n + 1):
        x = x0 + (x1 - x0) * i / n
        theta = 2.0 * math.pi * cyc * i / n + ph
        env = max(0.0, math.sin(theta)) ** 0.85         # inspiration gate
        y = y0 - amp * env * math.sin(2.0 * math.pi * carrier * i / n)
        pts.append(f"{x:.1f},{y:.1f}")
    return " ".join(pts)
...
def build_splash_svg(width: int = 780, height: int = 460, version: str | None = None) -> str:
  ...
  <polyline points="{_wave(0, w, 344, 26, 2.3, 1.6)}" fill="none" stroke="{_AZURE_SOFT}"
            stroke-width="2.6" stroke-opacity="0.6" stroke-linecap="round" stroke-linejoin="round"/>
  <!-- diaphragm EMG (orange): same length, gated to inspiration (blue crests) -->
  <polyline points="{_emg_di(0, w, 400, 26, 2.3, 1.6, 44)}" fill="none" stroke="{_ORANGE}"
            stroke-width="1.5" stroke-opacity="0.75" stroke-linecap="round" stroke-linejoin="round"/>
