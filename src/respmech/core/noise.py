...
class NoiseProfile:
    ...
    def apply(self, signal, prop_decrease: float):
        ...
        # Correct-phase reconstruction: scale each bin's magnitude only, keeping its
        # original phase exactly. The previous reconstruction (sign(Re z)*|masked| +
        # i*Im(z)*(1-mask)) treated the real and imaginary parts as independent
